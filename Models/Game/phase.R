...
for(prelevementProba in c(0.6,0.8,1.0)){for(predatorCarrying in c(0.01,0.015,0.02)){for(preyReproduction in c(0.01,0.015,0.02)){
  ...
  load(paste0(resfile,'_speeds.RData'))
  
 g=ggplot(df,aes(x=x,y=y))
 g+geom_segment(aes(xend = x + xs*ifelse(abs(xs)<10,2,0.8), yend = y+ ys*ifelse(abs(ys)<10,2,0.8),colour=abs(xs)+abs(ys)),
               arrow = arrow(length = unit(0.15,"cm")))+ scale_colour_gradient(low="green3",high="red",name="speed")
  #+ theme(axis.ticks = element_line(linetype = "dashed"), 
    #plot.background = element_rect(fill = "white"), 
  #  legend.position = "bottom", legend.direction = "horizontal")
 #  
  ggsave(filename=paste0(resfile,'_speed_latest.png'))
