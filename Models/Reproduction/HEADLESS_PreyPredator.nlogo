...
to setup-experiment [with-grass grass-r-time number-sheep number-wolves sheep-gain wolf-gain sheep-repro wolf-repro]
  ...
  set sheep-gain-from-food sheep-gain ; \in [0,50]
  set wolf-gain-from-food wolf-gain ; \in [0,100]
  set sheep-reproduce sheep-repro ; \in [1,20]
  set wolf-reproduce wolf-repro ; \in [0,20]
...
to go-experiment
  repeat max-ticks [
    if count turtles = 0 or count turtles > 100 * (initial-number-sheep + initial-number-wolves)[stop]
    go
  ]
end
...
to go
  ...
  if ticks mod 10 = 0 [set populations lput (list count sheep count wolves grass) populations]
